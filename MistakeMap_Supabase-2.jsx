...
// ── SESSION ID (prevents duplicate votes) ──
const SESSION_ID = (() => {
...
export default function App() {
  ...
  async function submitProblem(form) {
    const { error } = await supabase.from("problems").insert({
      ...
      name: form.name || "Anonymous", upvotes: 0
    });
            ...
            <div style={{ background: "#fff", border: "1px solid #dceadc", borderRadius: 10, padding: "1rem" }}>
              ...
              {indCounts.map(([ind, cnt]) => (
                <FilterBtn key={ind} active={curIndustry === ind} onClick={() => setCurIndustry(ind)}>
                  <span style={{ flex: 1, fontSize: "0.79rem" }}>{ind}</span>
                  <Pill>{cnt}</Pill>
                </FilterBtn>
              ))}
            </div>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "2rem 1.5rem", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3rem", alignItems: "start" }}>
          <div>
            ...
            <div style={{ display: "flex", gap: "0.5rem", marginTop: "1.25rem" }}>
              {["🇬🇧 United Kingdom", "🇺🇿 Uzbekistan"].map(c => <span key={c} style={{ background: "#f8faf8", border: "1px solid #dceadc", borderRadius: 6, padding: "0.38rem 0.7rem", fontSize: "0.82rem", fontWeight: 500 }}>{c}</span>)}
            </div>
          </div>
